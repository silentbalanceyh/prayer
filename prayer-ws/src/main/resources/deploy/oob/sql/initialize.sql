...
CREATE TABLE SYS_VCOLUMNS(
    K_ID VARCHAR(192),                          -- View中列元素ID标识符
    --Field属性
    S_NAME VARCHAR(256) NOT NULL,               -- 字段名称
    S_TYPE CHAR(16) NOT NULL                    -- 字段类型
        CHECK(S_TYPE='BooleanType' OR S_TYPE='IntType' OR S_TYPE='LongType' 
    ...
    D_COLUMN_NAME VARCHAR(256) NOT NULL,        -- 数据列名称
    D_COLUMN_TYPE VARCHAR(256) NOT NULL,        -- 数据列类型
    R_VIEW_ID VARCHAR(192),                     -- 关联SYS_VIEW表
    -- 关联属性
    PRIMARY KEY(K_ID),
...
CREATE TABLE SYS_TRIGGER(
    K_ID VARCHAR(192),                          -- TRIGGER的ID标识符
    D_NAME VARCHAR(256) NOT NULL,               -- 数据库中触发器名称：TG_前缀
    D_CATEGORY VARCHAR(20) NOT NULL,            -- 数据库类型，字符串：MSSQL, PGSQL, ORACLE, MYSQL
...
CREATE TABLE ENG_SCRIPT(
    K_ID VARCHAR(192),                          -- Script脚本的ID
    S_NAME VARCHAR(256) NOT NULL,               -- 脚本名称
    S_NAMESPACE VARCHAR(256) NOT NULL,          -- 脚本名空间
    S_CONTENT CLOB,                             -- 脚本内容
    PRIMARY KEY(K_ID)
);
--- ENG_SCRIPT
