...
public interface MsSqlStatement {
    ...
    /** INIT Server中查询当前表的字段被引用的References列表 **/
    ...
    /** INIT Server中查询外键引用表的信息 **/
    String R_RELATIONS = "SELECT F.TABLE_NAME AS FROM_TABLE,T.TABLE_NAME AS TO_TABLE FROM (INFORMATION_SCHEMA.REFERENTIAL_CONSTRAINTS AS B JOIN INFORMATION_SCHEMA.CONSTRAINT_COLUMN_USAGE AS F ON B.CONSTRAINT_NAME=F.CONSTRAINT_NAME JOIN INFORMATION_SCHEMA.CONSTRAINT_COLUMN_USAGE AS T ON T.CONSTRAINT_NAME=B.UNIQUE_CONSTRAINT_NAME) WHERE B.CONSTRAINT_CATALOG=''{0}''";
    // 测试表查询 =========================================
    /** INIT Server中读取所有测试表，以TST前缀，Prayer专用 **/
