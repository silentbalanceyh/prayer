...
public class OracleBuilder extends AbstractBuilder implements SqlSegment { // NOPMD
    ...
    /** **/
    @PostValidateThis
    public OracleBuilder(@NotNull @InstanceOf(Schema.class) final Schema schema) {
        super(schema);
        /**
     ...
     * (non-Javadoc)
     * 
     * @see com.prayer.facade.kernel.builder.AbstractBuilder#nullRows(java.lang.
     * String)
     */
    @Override
    ...
    protected String genAlterColumns(@NotNull final PEField field) {
        ...
        // 0. oracle dedicated handle for clob/blob fields, remove/add instead
        // of modify (ORA-22859: invalid modification of columns)
    ...
    private void genAlterColumnLines(final ConcurrentMap<StatusFlag, Collection<String>> statusMap, final Schema schema,
            final long rows) {
    ...
    private void genAddColumnLines(final ConcurrentMap<StatusFlag, Collection<String>> statusMap, final Schema schema,
            final long rows) {
    ...
    private void removeSeq() {
        ...
        if (MetaPolicy.INCREMENT == policy) {
            ...
            final String sql = OracleHelper.getSeqExist(this.getSchema().meta().getSeqName());
            final Long counter = this.getContext().count(sql);
            if (0 == counter) {
                info(LOGGER, "[I] Location: removeSeq(), Result : already is not existed");
            } else {
                removeSeqSql = MessageFormat.format(SEQ_REMOVE, this.getSchema().meta().getSeqName());
    ...
    private void createSeq() {
        ...
        if (MetaPolicy.INCREMENT == policy) {
            ...
            final Long counter = this.getContext().count(sql);
            if (0 == counter) {
                createSeqSql = MessageFormat.format(SEQ_CREATE, this.getSchema().meta().getSeqName(),
                        // fill with fixed values(min, max) for the time being
                        "1", "999999999", this.getSchema().meta().getSeqInit(), this.getSchema().meta().getSeqStep());
    ...
    /*
     * private String genIdentityLine(final MetaModel meta) { final
     * StringBuilder pkSql = new StringBuilder(); final FieldModel field =
     * this.getSchema().getPrimaryKeys().get(Constants.ZERO); // 1.1.主键字段和数据类型
     * final String columnType =
     * SqlDdlStatement.DB_TYPES.get(field.getColumnType()); // 2.字段名、数据类型，SQL
     * Server独有：NAME INT PRIMARY KEY IDENTITY
     * pkSql.append(field.getColumnName()).append(SPACE).append(columnType).
     * append(SPACE).append(MsSqlHelper.IDENTITY)
     * .append(BRACKET_SL).append(meta.getSeqInit()).append(COMMA).append(meta.
     * getSeqStep()) .append(BRACKET_SR); return pkSql.toString(); }
     */
    // ~ Get/Set =============================================
