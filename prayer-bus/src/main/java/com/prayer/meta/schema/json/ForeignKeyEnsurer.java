...
final class ForeignKeyEnsurer implements InternalEnsurer {
	...
	private boolean validateFKType() {
		// 21.1.验证外键的类型
		this.error = this.validator.verifyFkType(Attributes.F_TYPE, FK_FILTER,
				PK_STR);
		if (null != this.error) {
			return false; // NOPMD
		}
		// 21.2.验证外键的数据库列类型
		this.error = this.validator.verifyFkColumnType(Attributes.F_COL_TYPE,
				FK_FILTER, RGX_REFID_CTYPE);
