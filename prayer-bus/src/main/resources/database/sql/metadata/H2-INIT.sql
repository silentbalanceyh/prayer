...
--------------------------------------------------------------------------------------
--- EVX_PVRULE, 验证规则表，保存了某个接口参数的验证规则, Parameter Validation Rule
DROP TABLE IF EXISTS EVX_PVRULE;
CREATE TABLE EVX_PVRULE(
...
-- EVX_PVRULE的索引创建
CREATE INDEX IDX_PVRULE_NAME ON EVX_PVRULE(S_NAME);
CREATE INDEX IDX_PVRULE_TYPE ON EVX_PVRULE(S_TYPE);
CREATE INDEX IDX_PVRULE_ORDER ON EVX_PVRULE(S_ORDER);
CREATE INDEX RIDX_PVRULE_RULE_ID ON EVX_PVRULE(R_URI_ID);
